...
sealed class Result<S, F> {
  ...
  B fold<B>({
    required B Function(S s) ifSuccess,
    required B Function(F f) ifFailure,
  });
  ...
  Result<S2, F> map<S2>(S2 Function(S s) f) {
    return fold(ifSuccess: (S s) => success(f(s)), ifFailure: failure);
  }
  ...
  Result<S2, F> flatMap<S2>(Result<S2, F> Function(S s) f) {
    return fold(ifSuccess: f, ifFailure: failure);
  }
  ...
  Future<Result<S2, F>> asyncFlatMap<S2>(
    ...
    return fold(
      ifSuccess: f,
      ifFailure: (error) => Future.value(failure(error)),
    );
  }
